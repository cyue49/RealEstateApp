  ...
  const renderListingItem = ({ item }) => {
    ...
    const imageUrl =
      item.photoUrls && item.photoUrls.length > 0
        ? item.photoUrls[0]
        : require("../../assets/listing/listingPlaceholder.jpg");
    return (
      <TouchableOpacity
      ...
      >
        <Image
          ...
          onError={(error) =>
            console.error("Image load failed for:", imageUrl, error.nativeEvent)
          }
        />
        <View style={styles.listingInfo}>
          ...
          <Text style={styles.listingDetails}>
            {item.bedrooms}B + {item.bathrooms} Bath
          </Text>
        </View>
        <Ionicons
          name={monitoredListings ? "heart" : "heart-outline"}
          size={24}
          color={monitoredListings ? "red" : "gray"}
